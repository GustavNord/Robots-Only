...
def blink_text(text, blink_times=3, delay=0.5):
    """Simulates a blinking effect for text."""
...
class Character:
    ...
    def use_item(self, item):
        if item in self.inventory:
            if item == "Health Potion":
                ...
                type_effect(f"{self.name} used a Health Potion. Health is now {self.health}.")
            elif item == "Magic Wand":
                self.stats["Magic"] += 5
                self.inventory.remove(item)
                type_effect(f"{self.name} used the Magic Wand. Magic power increased!")
        else:
            type_effect("Item not in inventory.")
    ...
    def add_xp(self, xp):
        ...
        if self.xp >= 50 + (self.level - 1) * 50:  # Level up threshold: 50 XP per level, increases as level increases
...
class Enemy:
    def __init__(self, name, xp_reward):
        self.name = name
        self.health = 30  # All enemies now have 30 health
        self.strength = 5
...
class Game:
    ...
    def combat(self, enemy):
        ...
        while self.player.health > 0 and enemy.health > 0:
            ...
            if action == "1":
                ...
                time.sleep(1)  # Pause after item use
    ...
    def crossroads(self):
        ...
        clear_screen()
        type_effect("\nAfter defeating both enemies, you arrive at a crossroads.")
        time.sleep(1)
    ...
    def start_game(self):
        ...
        type_effect("Welcome to Quest for the Lost Relic!")
